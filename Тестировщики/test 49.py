...
class King(Piece):  
    ...
    def Moves(self):
        moves = []
        impos = Opponent_moves(self.color)
        if(0 <= self.x - 1 <= 7 and 0 <= self.y - 1 <= 7):
            if(Board[self.x - 1][self.y - 1] == '.' and [self.x - 1, self.y - 1] not in impos):
        ...
        if(0 <= self.y - 1 <= 7 and Board[self.x][self.y - 1] == '.' and [self.x, self.y - 1] not in impos):
        ...
        if(0 <= self.x - 1 <= 7 and Board[self.x - 1][self.y] == '.' and [self.x - 1, self.y] not in impos):
        ...
        if(0 <= self.x + 1 <= 7 and Board[self.x + 1][self.y] == '.'):
            if(Board[self.x + 1][self.y] == '.' and [self.x + 1, self.y] not in impos):
                moves.append([self.x + 1, self.y])
            else:
                if(abs(Board[self.x + 1][self.y].color - self.color) == 1):
                    if Pos_beat(self.color, self.x + 1, self.y) == 1:
        ...
        if(0 <= self.y + 1 <= 7):
            if(Board[self.x][self.y + 1] == '.' and [self.x, self.y + 1] not in impos):
                moves.append([self.x, self.y + 1])
            else:
                if(abs(Board[self.x][self.y + 1].color - self.color) == 1):
                    if Pos_beat(self.color, self.x, self.y + 1) == 1:
