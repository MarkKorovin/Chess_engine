...
class Knight(Piece):
    ...
    def Moves(self):
        ...
        if Knight.Take_Move(self.color, self.x - 1, self.y - 2) == 1:
            moves.append([self.x - 1, self.y - 2])

        if Knight.Take_Move(self.color, self.x - 2, self.y - 1) == 1:
...
class King(Piece):  
    ...
    def Moves(self):
        moves = []
        if King.Take_Move(self.color, self.x - 1, self.y - 1) == 1:
            moves.append([self.x - 1, self.y - 1])
            
        if King.Take_Move(self.color, self.x, self.y - 1) == 1:
            moves.append([self.x, self.y - 1])

        if King.Take_Move(self.color, self.x + 1, self.y - 1) == 1:
            moves.append([self.x + 1, self.y - 1])
            
        if King.Take_Move(self.color, self.x - 1, self.y) == 1:
            moves.append([self.x - 1, self.y])
