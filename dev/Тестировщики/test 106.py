...
for i in range(8):
    Board[i] = ['.', '.', '.', '.', '.', '.', '.', '.']
def FEN_get():
    FEN = 'rnb2bnr/4kp2/q1p4p/1p1pp1p1/p2PPP1P/1PN2K2/P1PQN1PR/1RB2B2'
    i = 0
    x = 0
    y = 0
    while i < len(FEN):
        if FEN[i] == '/':
            ...
            Color = round(3.1074766355 - 0.0280373832 * ord(FEN[i]))
            if 49 <= ord(FEN[i]) <= 56:
                y += int(FEN[i])
            if FEN[i] == 'r' or FEN[i] == 'R':
                Board[x][y] = Rook(Color, x, y)
            if FEN[i] == 'k' or FEN[i] == 'K':
                Board[x][y] = King(Color, x, y)
            if FEN[i] == 'b' or FEN[i] == 'B':
                Board[x][y] = Bishop(Color, x, y)
            if FEN[i] == 'p' or FEN[i] == 'P':
                Board[x][y] = Pawn(Color, x, y)
            if FEN[i] == 'q' or FEN[i] == 'Q':
                Board[x][y] = Queen(Color, x, y)
            if FEN[i] == 'n' or FEN[i] == 'N':
                Board[x][y] = Knight(Color, x, y)
            y += 1
        i += 1
def Print_Board():
    Print_Board = ''
