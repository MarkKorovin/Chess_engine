...
def King_check(color):
    ...
    while i < len(moves) - 1:
        while j < len(moves[i] - 1):
            Board[moves[i][j]][moves[i][j+1]] = Board[moves[i][0]][moves[i][1]]
            Board[moves[i][0]][moves[i][1]] = '.'
            Board[moves[i][j]][moves[i][j+1]].x += moves[i][j] - moves[i][0]
            Board[moves[i][j]][moves[i][j+1]].y += moves[i][j] - moves[i][1]
            if Check(color) == 0:
                oxt = [moves[i][0], moves[i][1], moves[i][j], moves[i][j+1]]
                lst.append(oxt)
            j += 2
            Board = old_Board
        j = 2
