...
def Game_result(move_turn, moves, move):
    flag = 0
    global moves50
    result = '*'
    ...
    if Board[move[0]][move[1]].__str__() not in {'p', 'P'}:
        ...
        moves50 = 0
    if Board[move[2]][move[3]] not in {'p', 'P', 'q', 'Q', 'k', 'K', 'n', 'N', 'b', 'B', 'r', 'R'}:
        if flag != 1:
            moves50 += 1
    else:
        moves50 = 0
...
while result == '*':
    ...
    result = Game_result(move_turn, cur_moves, move)
    if result != '*':
        break
    Do_Move(move)
