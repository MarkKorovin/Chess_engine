...
def Possible_moves(color):
    global Board
    lst = Color_moves(color)
    moves = []
    i = 0
    j = 2
    while i < (len(lst) - 1):
        while j < (len(lst[i]) - 1):
            Board[lst[i][j]][lst[i][j + 1]] = Board[lst[i][0]][lst[i][1]]
            Board[lst[i][0]][lst[i][1]] = '.'
            Board[lst[i][j]][lst[i][j + 1]].x = lst[i][j]
            Board[lst[i][j]][lst[i][j + 1]].y = lst[i][j + 1]
            print(Print_Board())
            if Check(color) == 0:
                moves.append([lst[i][0], lst[i][1], lst[i][j], lst[i][j + 1]])
                print('72346H24762H34293746H23976429')
            Board[lst[i][0]][lst[i][1]] = Board[lst[i][j]][lst[i][j + 1]]
            Board[lst[i][j]][lst[i][j + 1]] = '.'
            Board[lst[i][0]][lst[i][1]].x = lst[i][0]
            Board[lst[i][0]][lst[i][1]].y = lst[i][1]
            print(Print_Board())
            j += 2
        i += 1
    return moves
